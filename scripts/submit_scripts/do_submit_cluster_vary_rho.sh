...
rhos=(2.000000 3.000000 4.000000 5.000000 10.000000)
Ebs=(7.500000)
Ecs=(0.000000 1.000000)
                ...
                for Ec in "${Ecs[@]}"; do
                    for rho in "${rhos[@]}"; do
                        for seed in "${seeds[@]}"; do
                            input_file=$SCRATCH/capsid-assembly/llps/droplet/assembly_trajectories/N=${N}/L=${L}/Vr=${Vr}/E_cond=${Ec}/E_bond=${Eb}/rho=${rho}/seed=${seed}/traj.gsd

                            out_file=$SCRATCH/capsid-assembly/llps/droplet/out_files/cluster_N=${N}_L=${L}_Vr=${Vr}_Eb=${Eb}_Ec=${Ec}_rho=${rho}_seed=${seed}.out
