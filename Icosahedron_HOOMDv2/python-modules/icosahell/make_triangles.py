...
def make_triangles( N_rows, N_per_edge, vertices, patch_dist,
    ...
    for edge in range(0, 3):
        ...
        for j in range(N_patches):
            xp_left  = xh - rrrr*(0.75+0.25*j)
            xp_right = xh + rrrr*(0.75+0.25*j)
            X[i] = xp_left
            X[i+1] = xp_right
            i += 2
    print("is is now at", i, file = sys.stderr)
    with open("triangle.pts", "w") as fp:
        for x in X:
            print(x[0], x[1], x[2], file = fp)
    ...
    X = np.array( tmp )
    with open("triangle_filter.pts", "w") as fp:
        for x in X:
            print(x[0], x[1], x[2], file = fp)
