...
def generate_lattice( N, x0, a1, a2, a3, Nx, Ny, Nz ):
...
def generate_cubic( N, fac ):
    """ Generates the skeleton (T1 structure). """
    Nx = Ny = Nz = int( math.ceil( N ** (1.0/3.0) ) )
    X = generate_lattice( N,
                          fac*np.array( [ 0.5, 0.5, 0.5 ] ),
                          np.array( [ fac, 0.0, 0.0 ] ),
                          np.array( [ 0.0, fac, 0.0 ] ),
                          np.array( [ 0.0, 0.0, fac ] ),
                          Nx, Ny, Nz )
    return X, np.ones( N )
